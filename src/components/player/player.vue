...
import { unescapeHTML } from 'common/js/dom'
...
export default {
  ...
  data() {
    return {
      songReady: false, //节流阀，避免快速点击出现异常
      currentTime: 0,
      radius: 32,
      currentLyric: null,
      currentLineNum: 0,
  ...
  methods: {
    ...
    leave(el, done) {
      ...
      this.$refs.cdWrapper.style.webkitTransition = 'all 0.4s'
      const { x, y, scale } = this._getPosAndScale()
    ...
    afterLeave() {
      ...
      this.$refs.cdWrapper.style.webkitTransition = ''
      this.$refs.cdWrapper.style['transform'] = ''
      this.$refs.cdWrapper.style['webkitTransform'] = ''
    },
    togglePlaying() {
    ...
    loop() {
      ...
      if (this.currentLyric) {
        this.currentLyric.seek(0) //跳到初始位置
      }
    },
    ...
    middleTouchMove(e) {
      ...
      this.$refs.lyricList.$el.style['transition'] = 'all 0 ease-in-out'
      this.$refs.lyricList.$el.style['webkitTransition'] = 'all 0 ease-in-out'
      this.$refs.lyricList.$el.style['transform'] =
        'translate3d(' + offsetWidth + ')'
    ...
    middleTouchEnd(e) {
      ...
      this.$refs.lyricList.$el.style['transition'] =
        'all ' + time + 'ms ease-in-out'
      this.$refs.lyricList.$el.style['webkitTransition'] =
        'all ' + time + 'ms ease-in-out'
      this.$refs.lyricList.$el.style['transform'] =
        'translate3d(' + offsetWidth + ')'
      this.$refs.lyricList.$el.style['webkitTransform'] =
  ...
  watch: {
    currentSong(newSong, oldSong) {
      ...
      getLyric(newSong.id).then(res => {
        res = JSON.parse(res.replace('MusicJsonCallback(', '').replace(')', ''))
        if (res.code === ERR_OK) {
          lyric = unescapeHTML(res.lyric)
        }
      })
      ...
      this.timer=setTimeout(() => {
        this.$refs.audio.play()
        //获取歌词
        this.currentLyric = new Lyric(lyric.innerHTML, this.handleLyric)
        if (this.playing) {
